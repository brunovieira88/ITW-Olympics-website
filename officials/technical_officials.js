...
var vm = function () {
    ...
    self.setFavorite = function () {
      var ident = this.Id;
      var existingInfo = JSON.parse((localStorage.getItem("fav_officials")) || "[]");
      ...
      if (count != 1) {
        ...
        existingInfo = existingInfo.filter(function (official) {
        return official.Id !== ident;
      });
        $("#" + ident).html("<i class='fa fa-heart-o' aria-hidden='true'></i>");
      }
      localStorage.setItem("fav_officials", JSON.stringify(existingInfo));
      console.log(JSON.parse(localStorage.getItem("fav_officials") || "[]"));
    };
    // tirar refreh se der
    self.removeAllFavorites = function () {
      localStorage.removeItem("fav_officials");
    ...
    self.activate = function (id) { 
      console.log("CALL: getThechnical_Officials...");
      var composedUri =
      ...
      ajaxHelper(composedUri, "GET").done(function (data) {
        ...
        allThechnical_officials = data.Technical_officials;
      });
    };
    //--- Internal functions
  ...
  init: function (element, valueAccessor) {
    ...
    $(element).click(function () {
      if (value.isShowingFav()) {
        value.technical_officials(allThechnical_officials);
        value.isShowingFav(false);
        $("#Pag").removeClass("d-none");
      }
      else {
        let favoritesOfficials = JSON.parse(localStorage.getItem("fav_officials") || "[]");
        value.technical_officials(favoritesOfficials);
